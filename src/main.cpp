...
uint256 hashGenesisBlock("0xeb1817e12a0d22c3213cb0cbe477be11b450c75c77202a071b63d8fc645d4bda");
...
int64 nChainStartTime = 1394028008;
...
const string strMessageMagic = "Emucoin Signed Message:\n";
...
{
    // Emucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 9)
    {
        nSubsidy = 504000000 * COIN; // Premine at block 9 to avoid problems at beginning
    }
    else if(nHeight < 60)
    {
        nSubsidy = 0; // Coins maturation
    }
    else if(nHeight < 50000)
    {
        rand0 = generateMTRandom(seed, 799999);
        nSubsidy = (1 + rand0) * COIN;
    }
    else if(nHeight < 100000)
    {
        ...
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 399999);
    ...
    else if(nHeight < 200000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand2 = generateMTRandom(seed, 199999);
    ...
    else if(nHeight < 300000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand3 = generateMTRandom(seed, 99999);
    ...
    else if(nHeight < 400000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand4 = generateMTRandom(seed, 49999);
    ...
    else if(nHeight < 500000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand5 = generateMTRandom(seed, 24999);
...
static const int64 nTargetTimespan = 8 * 60 * 60;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Emucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Emucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Emucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    ...
    {        
        ...
        pchMessageStart[2] = 'u';
        pchMessageStart[3] = 's';
        hashGenesisBlock = uint256("0xf919ffa28a0293a397663732c887b82902de3ed5f941dc8d1eeace808c2049ba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "March 2014 Reign of the EMUs begin ahahaha ououououh";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 1081466;
        block.nTime = 1394028008;
        
        if (fTestNet)
        {

            block.nNonce = 99213;
            block.nTime = 1394027693;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x54986d4eef5c8273573dfef94b7df9d79dc26aa7fc8eb6b96563fafc75f44651"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Emucoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmucoinMiner(CWallet *pwallet)
{
    printf("EmucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Emucoin-miner");
    ...
    try { loop {
        ...
        printf("Running EmucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmucoinMiner, pwallet));
}
